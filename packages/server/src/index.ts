...
export class App {
    ...
    async config() {
            ...
            this.app.use(async (req, res, next) => {
                ...
                if (URL_CASE_INSENSITIVE_REGEX.test(req.path)) {
                    ...
                    if (URL_CASE_SENSITIVE_REGEX.test(req.path)) {
                        ...
                        // ==========|| custom code ||==========
                        const isWhitelisted = whitelistURLs.some((url) => req.path.includes(url))
        ...
        // ==========|| custom code ||==========
        this.app.use(`${baseUrl}/api/v1`, flowiseApiV1Router)
        ...
        // ==========|| org code ||==========
        // this.app.use('/test', express.static(uiBuildPath))

        // ==========|| custom code ||==========
        this.app.use(baseUrl || '/', express.static(uiBuildPath))
        ...
        // ==========|| custom code ||==========
        this.app.get(`${baseUrl}/*`, (req: Request, res: Response) => {
          res.sendFile(uiHtmlPath)
         })
...
export async function start(): Promise<void> {
    ...
    // ==========|| custom code ||==========
    const path = process.env.FLOWISE_BASE_URL ?? ''
    ...
    server.listen(port, host, () => {
        ...
        // ==========|| custom code ||==========
        logger.info(`⚡️ [server]: Flowise Server is listening at ${host ? 'http://' + host : ''}:${port}${path}`)
    })
}
